...
/// Convert enums to 32-bit representations, but skip model enums.
class EnumOn32Bits : public P4::ChooseEnumRepresentation {
    bool convert(const IR::Type_Enum *type) const override {
        if (type->srcInfo.isValid()) {
            auto sourceFile = type->srcInfo.getSourceFile();
            if (sourceFile.endsWith("_model.p4")) return false;
        }
        return true;
    }
...
const IR::ToplevelBlock *MidEnd::run(DslxOptions &options, const IR::P4Program *program,
    ...
    if (options.loadIRFromJson == false) {
        ...
        midEnd.addPasses(
            {new P4::ValidateTableProperties({"size"_cs, "implementation"_cs})});
