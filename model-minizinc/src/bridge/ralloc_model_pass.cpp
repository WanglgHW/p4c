...
const IR::Node *RallocModelPass::preorder(IR::BFN::Pipe *pipe) {
    ...
    // advisory and re-invoking the solver each round is wasteful, so solve only
    // once and delegate to the legacy allocator on every subsequent (backtrack) round.
    ...
    try {
        ...
        // then turns model_input.json into model_out.json via MiniZinc/CP-SAT.
        ...
        // resume bundle by stable name) instead of running the solver in-process.
