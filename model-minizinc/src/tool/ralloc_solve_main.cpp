 ...
 * M3 (VLIW/action) via MiniZinc + OR-Tools CP-SAT, and writes a model_out.json
 * (ralloc::ModelResults) for the compiler's resume phase. This is the ONLY
 * process that shells out to the solver, so it never executes inside
...
int main(int argc, char **argv) {
    ...
    if (!ralloc::hasMiniZinc()) {
        std::cerr << "ralloc-solve: minizinc not found — cannot solve (install MiniZinc "
                     "with an OR-Tools CP-SAT backend, or set $RALLOC_MINIZINC)\n";
        return 1;
    }
