# ralloc — Tofino Resource Allocation via MILP (SCIP)
...
with a **Mixed-Integer Linear Programming** model solved by **SCIP**, then writes
...
| `doc/05_scip_integration.md` | SCIP encoding, decomposition, warm-start, coupling cuts |
...
include/ralloc/
  ...
  scip_solver.h      SolverBackend + ScipBackend
...
test/                structural unit tests (no SCIP, no compiler)
```

## Build (standalone, no compiler)

```bash
cmake -S model -B model/build            # add -DSCIP_DIR=... to enable solving
cmake --build model/build
ctest --test-dir model/build             # runs ralloc_unit
```

Without SCIP the core still builds; the solver becomes an `.lp` exporter and the
in-compiler path falls back to the legacy allocator.
...
- `CMakeLists.txt` builds a `ralloc_bridge` static library from the `model/`
  ...
  SCIP is auto-detected (`find_package(SCIP CONFIG)`); without it the solver
  degrades to `.lp` export and the pass always falls back.
...
SCIP can also run as a **separate process** from the compiler, handing off via
three JSON files (see `doc/08_decoupled_flow.md`):
...
model/build/ralloc-solve /tmp/io/model_input.json -o /tmp/io/model_out.json       # SCIP runs HERE only
...
SCIP-under-p4c-GC re-entrancy hazard. Verified end-to-end on
...
SCIP, and was tested end-to-end** on a TNA program (see `doc/MANUAL.md`):

- `--use-ralloc` is wired into `backend.cpp`; with it set, the pass ingests the
  real IR, builds the MILP, and **SCIP solves M2 (MAU placement) + M3 (VLIW) to
  proven optimality** (`SCIP Status: optimal solution found`,
  `ralloc: MILP solved — N stage(s), M table(s)`).
...
model builders, and the SCIP backend.
