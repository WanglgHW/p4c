...
API (`model/include/ralloc/resource_model.h`), the pass that hosts it, how solver
results are written back into the backend's native structures, and the fallback
contract that guarantees the model can never regress compilability.
...
structs** (`model/include/ralloc/model_types.h`):
...
> (`BFN::RallocModelPass`, `model/src/bridge/ralloc_model_pass.cpp`), gated by
...
```cpp
// model/include/ralloc/ralloc_model_pass.h
...
The standalone solver (`ralloc-solve`, `model/src/tool/ralloc_solve_main.cpp`) then
reads `model_input.json` and writes `model_out.json` — see Doc 08.

## 3. The public API (`resource_model.h`)

The header (delivered in `model/include/ralloc/resource_model.h`) declares:
