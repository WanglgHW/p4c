...
experimental MILP/SCIP resource allocator (`--use-ralloc`) in `bf-p4c`.

- **What it is:** a Mixed-Integer Linear Programming model of Tofino MAU resource
  allocation, solved by [SCIP](https://www.scipopt.org/), integrated into the
  compiler as `BFN::RallocModelPass`.
- **Status:** experimental, **off by default**, runs in **advisory mode** (see
  ...
  output.
- **Two ways to run it:** *in-process* (`--use-ralloc`, §3) where SCIP runs inside
  `p4c-barefoot`; or the *decoupled out-of-process flow* (`--ralloc-emit` /
  `ralloc-solve` / `--ralloc-resume`, **§10**) where the solver is a separate
  process exchanging three JSON files. §11 is a worked example that verifies the
...
| **SCIP** (optional but required to actually solve) | e.g. `brew install scip`; without it the pass degrades to `.lp` export and always falls back |

Check SCIP is visible to CMake:

```bash
ls $(brew --prefix)/lib/cmake/scip/scip-config.cmake   # macOS/homebrew
```
...
`backends/tofino/bf-p4c/CMakeLists.txt`. To pick it up you must **reconfigure**
so CMake finds SCIP and builds the `ralloc_bridge` library.

```bash
# from the build directory
cmake . -DCMAKE_PREFIX_PATH=$(brew --prefix)      # finds SCIP
# expect: "-- ralloc: SCIP found — MILP solving enabled"
...
- If SCIP is **not** found you'll see `ralloc: SCIP not found — .lp export only`.
  The build still succeeds; `--use-ralloc` will just always fall back.
...
SCIP prints its own solve banner to stdout/stderr (status, nodes, bound, gap).

### Example output (success)

```
SCIP Status        : problem is solved [optimal solution found]   # M2 MAU
SCIP Status        : problem is solved [optimal solution found]   # M3 VLIW
...
The MILP solves two sub-models, both visible via the SCIP banners:
...
`optimal` means SCIP closed the gap; `feasible` means it found a valid allocation
...
2. solve M2 + M3 with SCIP (and log the optimal placement),
...
| `ralloc: SCIP not found` at configure | install SCIP and reconfigure with `-DCMAKE_PREFIX_PATH=$(brew --prefix)` (or set `SCIP_DIR`). |
...
| `ralloc-solve: SCIP not linked` | the standalone model build didn't find SCIP; rebuild `model/build` with `-DCMAKE_PREFIX_PATH=$(brew --prefix)`. |
...
| `model/src/solver/scip_solver.cpp` | SCIP backend |
...
```bash
# configure (once, to pick up SCIP + ralloc)
...
allocator — which also sidesteps the SCIP-under-p4c-GC re-entrancy hazard) and
...
`ralloc-solve` links only the model core (which pulls SCIP), so it builds from the
...
#   writes $IO/model_input.json and $IO/ir_middle.json; no SCIP runs here.

# Phase 2 — solve out-of-process (this is the ONLY process that runs SCIP).
...
Exit code is non-zero if the solve is unusable (infeasible/timeout/no SCIP), so a
...
- **Emit** (`RallocModelPass::preorder`): builds the same `LayoutChoices` /
  ...
  exit. **No SCIP runs in this process.**
...
This is the procedure used to confirm the model/SCIP path is correct and
non-regressing, on `testdata/dpvs-l4LB/dpvs_l4lb.p4`.

### 11.1 Compile twice — original algorithm vs. model/SCIP
...
# RUN 2 — MODEL/SCIP method (decoupled 3-phase)
...
actual binary/layout is identical** ⇒ the model/SCIP pipeline runs end-to-end and
...
| | Original (legacy greedy) | Model/SCIP |
|---|---|---|
...
1. **The decoupled model/SCIP infrastructure is correct** — emit → solve → resume
...
In short: the new model/SCIP way is verified **OK** (correct and non-regressing),
